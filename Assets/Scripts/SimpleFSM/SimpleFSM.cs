...
public class SimpleFSM : FSM 
{
    ...
    protected override void FSMUpdate()
    {
        ...
        { 
            print(hitCollider);
            if (this.gameObject != hitCollider.gameObject && hitCollider.gameObject.tag == "Enemy"){
                print("collison!");
                elapsedTime = 0f;
    ...
    protected void UpdateAttackState()
    {
        ...
        float dist = Vector3.Distance(transform.position, playerTransform.position);
        if(dist <= 150.0f && health < 100){
            curState = FSMState.Flee;
        } else if (dist >= 200.0f && dist < 300.0f)
        {
            //Rotate to the target point
            Quaternion targetRotation = Quaternion.LookRotation(destPos - transform.position);
            transform.rotation = Quaternion.Slerp(transform.rotation, targetRotation, Time.deltaTime * curRotSpeed);  
    ...
    protected void UpdateFleeState(){
        ...
        if(dist <= fleeingDistance){
            ...
            Vector3 directionTowardsPlayerTank = (playerTransform.position - transform.position).normalized;
            Vector3 directionOppositeToPlayerTank = -directionTowardsPlayerTank * fleeingDistance;
            destPos = directionOppositeToPlayerTank;

            //This rotates the enemy tank towards the direction opposite to the player tank.
            ...
            //if out of range, change to patrol
            curState = FSMState.Patrol;
        }
    }
    ...
    protected void UpdateEvadeState(){
        ...
        //Rotate tank away
        transform.rotation *= Quaternion.Euler(0, 90f * curRotSpeed * Time.deltaTime, 0);
